package haiphat.com.bds.nghetuvan.services

class Config {
    companion object {
        const val API_URL: String = "http://app-bds.haiphatland.com.vn/api"
        const val API_URL_OAUTH = "http://app-bds.haiphatland.com.vn"
        const val API_URL_MEDIA ="http://media.haiphatland.com.vn/api"
        const val API_URL_CRM ="http://quanlyhocvien.ml/api"
        const val API_URL_HPACADEMY ="http://hpacademy.com.vn/api"

        const val CLIENT_ID = "2"
        const val CLIENT_SECRET = "tbSp7MgFTTYGPBxYILmTAofarMBeZuO9jo2EuLL3"
        const val CRM = 1
        const val HPACADEMY = 2
    }
}